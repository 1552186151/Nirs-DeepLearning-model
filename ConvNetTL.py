...
class ConvNetTL(nn.Module):
    def __init__(self):
        super(ConvNetTL, self).__init__()
        ...
        # self.AvgPool = nn.AdaptiveAvgPool1d(output_size = 128) # GAP全局池化层
        self.mlp = nn.Sequential(
            nn.Linear(8192, 128),  # 4480 8960 17920 全连接层,
            nn.Linear(128, 64),
            nn.Linear(64, 1)
        )
        # self.fc1 = nn.Linear(8192, 128)  # 4480 8960 17920 全连接层
        # self.fc2 = nn.Linear(128, 64)
        # self.fc3 = nn.Linear(64, 1)
        self.drop = nn.Dropout(0.2)  # 每个神经元都有p概率的可能性置零

    def forward(self, out):
        out = self.conv1(out)
        # out = self.drop(out)
        out = self.conv2(out)
        out = self.conv3(out) # out.shape[16, 64, 2057]
...
class DeepSpectraTL(nn.Module):
    def __init__(self):
        super(DeepSpectraTL, self).__init__()
        self.conv1 = nn.Sequential(
